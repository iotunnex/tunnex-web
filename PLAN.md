...
Current: **S4.2 merged — next: S4.3 (legal + analytics)**
...
- **SECURITY — Turnstile secret rotation: DONE (2026-07-12).** Regenerated by Pawan,
  synced to the Worker, deployed; temp secret-sync workflow removed. Future secret
  changes: set the repo secret, then any main deploy carries it (wrangler deploy
  inherits the current secret set).
